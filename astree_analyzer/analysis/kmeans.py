...
if (
    df.shape[0] < 2
    or "CountCommit" not in df.columns
    or "CountComment" not in df.columns
):
    raise ValueError(
        "The dataset must contain at least two samples and both 'CountCommit' and 'CountComment' columns for clustering."
    )
...
if scaled_data.shape[0] > 1:
    ...
    plt.scatter(
        pca_df["PC1"], pca_df["PC2"], c=pca_df["Cluster"], cmap="viridis", marker="o"
    )
