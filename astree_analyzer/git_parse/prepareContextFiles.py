...
def write_context_to_file(directory, commit_hash, file_name, line_number, context, code, suffix):
    context_str = "\n".join(context)
    file_content = f"/* Commit: {commit_hash}\n * File: {file_name}\n * Line: {line_number}\n * Code: {code}\n */\n\n{context_str}"
    
    output_file_path = Path(directory) / f"{commit_hash}_{file_name}_{line_number}_{suffix}.c"
...
for commit_hash, files in comments_data.items():
    for file_info in files:
        ...
        for line_number, comment_info in comments.items():
            ...
            if context:
                write_context_to_file(output_comments_dir, commit_hash, file_name, line_number, context, code, 'comment')
...
for commit_hash, files in commits_data.items():
    for file_info in files:
        ...
        for line_number, line_info in removed_lines.items():
            ...
            code = line_info['code']
            if context:
                write_context_to_file(output_commits_dir, commit_hash, file_name, line_number, context, code, 'commit')
