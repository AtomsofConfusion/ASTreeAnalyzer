...
class TestASTSerializer(unittest.TestCase):
    ...
    def test_serialize_var_decl(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="var", type_spelling="int")
        result = self.serializer._serialize_node(cursor)
        self.assertIn("var_0_int", result)

    def test_serialize_function_decl(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.FUNCTION_DECL, spelling="func", type_spelling="void")
        result = self.serializer._serialize_node(cursor)
    ...
    def test_serialize_call_expr(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.CALL_EXPR, spelling="call")
    ...
    def test_serialize_decl_ref_expr(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.DECL_REF_EXPR, spelling="ref", type_spelling="int")
        result = self.serializer._serialize_node(cursor)
        self.assertIn("var_0_int", result)

    def test_serialize_parm_decl(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.PARM_DECL, spelling="param", type_spelling="int")
        result = self.serializer._serialize_node(cursor)
        self.assertIn("var_0_int", result)

    def test_serialize_field_decl(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.FIELD_DECL, spelling="field", type_spelling="int")
        result = self.serializer._serialize_node(cursor)
        self.assertIn("var_0_int", result)

    def test_serialize_unknown_template_argument(self):
        cursor = self.create_mock_cursor(ExtendedCursorKind.UNKNOWN_TEMPLATE_ARGUMENT_KIND)
        result = self.serializer._serialize_node(cursor)
    ...
    def test_serialize_identifier_kind(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="id", type_spelling="int")
        result = self.serializer._serialize_node(cursor)
        self.assertIn("var_0_int", result)
    ...
    def test_serialize_call_expr_child(self):
        parent = self.create_mock_cursor(clang.cindex.CursorKind.CALL_EXPR, spelling="call")
    ...
    def test_serialize_with_nested_children(self):
        parent = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="var", type_spelling="int")
        child = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="nested_var", type_spelling="int")
    ...
    def test_serialize_function_call(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.CALL_EXPR, spelling="func")
    ...
    def test_serialize_if_stmt(self):
        ...
        cursor.get_children = MagicMock(return_value=iter([condition, then_branch, else_branch]))
        result = self.serializer._serialize_node(cursor)
        self.assertIn("CursorKind.IF_STMT(CursorKind.BINARY_OPERATOR,CursorKind.COMPOUND_STMT,CursorKind.COMPOUND_STMT)", result)
    ...
    def test_serialize_for_stmt(self):
        ...
        cursor.get_children = MagicMock(return_value=iter([init, condition, increment, body]))
        result = self.serializer._serialize_node(cursor)
        self.assertIn("CursorKind.FOR_STMT(CursorKind.DECL_STMT,CursorKind.BINARY_OPERATOR,CursorKind.UNARY_OPERATOR_++_pre,CursorKind.COMPOUND_STMT)", result)
    ...
    def test_serialize_for_with_nested_if_stmt(self):
        ...
        cursor.get_children = MagicMock(return_value=iter([init, condition, increment, body]))
        result = self.serializer._serialize_node(cursor)
        self.assertIn("CursorKind.FOR_STMT(CursorKind.DECL_STMT,CursorKind.BINARY_OPERATOR,CursorKind.UNARY_OPERATOR_++_pre,CursorKind.IF_STMT)", result)

    def test_serialize_function_decl_with_params(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.FUNCTION_DECL, spelling="func", type_spelling="void")
        param1 = self.create_mock_cursor(clang.cindex.CursorKind.PARM_DECL, spelling="param1", type_spelling="int")
        param2 = self.create_mock_cursor(clang.cindex.CursorKind.PARM_DECL, spelling="param2", type_spelling="float")
    ...
    def test_serialize_decl_stmt(self):
        ...
        decl = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="var", type_spelling="int")
    ...
    def test_serialize_decl_stmt_with_multiple_var_decl(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.DECL_STMT)
        var_decl1 = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="var1", type_spelling="int")
        var_decl2 = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="var2", type_spelling="float")
    ...
    def test_serialize_decl_stmt_with_var_decl(self):
        cursor = self.create_mock_cursor(clang.cindex.CursorKind.DECL_STMT)
        var_decl = self.create_mock_cursor(clang.cindex.CursorKind.VAR_DECL, spelling="var", type_spelling="int")
    ...
    def test_serialize_if_with_nested_compound_stmt(self):
        ...
        result = self.serializer._serialize_node(cursor)
        self.assertIn("CursorKind.IF_STMT(CursorKind.BINARY_OPERATOR,CursorKind.COMPOUND_STMT)", result)
